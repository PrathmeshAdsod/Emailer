...
// Initialization of the Nylas SDK using the client credentials
Nylas.config( {
...
const CLIENT_URI =
  process.env.CLIENT_URI || `http://localhost:${process.env.FRONTEND_PORT || 5000}`;
...
.then((applicationDetails) => {
  console.log(
    'Application IS SUCCESSFULLY registered. \n Application Details: ',
...
app.post('/nylas/completion', async(req, res) => {
  async function chatCompletion() {
    const openai = new OpenAI({
      apiKey: process.env.OPENAI_API_KEY,
    });

    const prompt = 'what is meant by prompt'
